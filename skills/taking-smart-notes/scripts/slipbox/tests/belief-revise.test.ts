...
describe("slipbox belief revise", () => {
  test("appends refined entry, bumps last_reviewed", async () => {
    ...
    const r =
      await $`bun run ${cli} belief revise b1 --note="Brooks introduced X; now hold Y' instead of Y"`
        .cwd(tmp)
        .nothrow()
        .quiet();
  ...
  test("--status=retired updates frontmatter and log", async () => {
    const { tmp, cli } = await setup();
    const r = await $`bun run ${cli} belief revise b1 --status=retired --note="No longer applies"`
      .cwd(tmp)
      .nothrow()
      .quiet();
    expect(r.exitCode).toBe(0);
  ...
  test("--status=superseded requires --superseded-by", async () => {
    const { tmp, cli } = await setup();
    const r = await $`bun run ${cli} belief revise b1 --status=superseded --note="Replaced"`
      .cwd(tmp)
      .nothrow()
      .quiet();
  ...
  test("--superseded-by sets the field", async () => {
    ...
    await $`bun run ${cli} belief new b2 --scope=personal --title="B2" --falsifier="F"`
      ...
      .quiet();
    const r =
      await $`bun run ${cli} belief revise b1 --status=superseded --superseded-by=b2 --note="Replaced by b2"`
        .cwd(tmp)
        .nothrow()
        .quiet();
    expect(r.exitCode).toBe(0);
  ...
  test("--superseded-by must resolve to an existing belief", async () => {
    const { tmp, cli } = await setup();
    const r =
      await $`bun run ${cli} belief revise b1 --status=superseded --superseded-by=ghost --note="x"`
        .cwd(tmp)
        .nothrow()
        .quiet();
  ...
  test("rejects self-supersede", async () => {
    const { tmp, cli } = await setup();
    const r =
      await $`bun run ${cli} belief revise b1 --status=superseded --superseded-by=b1 --note="x"`
        .cwd(tmp)
        .nothrow()
        .quiet();
    expect(r.exitCode).toBe(2);
  ...
  test("rejects unknown --status value", async () => {
    const { tmp, cli } = await setup();
    const r = await $`bun run ${cli} belief revise b1 --status=bogus --note="x"`
      .cwd(tmp)
      .nothrow()
      .quiet();
    expect(r.exitCode).toBe(2);
